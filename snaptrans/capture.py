...
class ResizablePanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._min_w, self._min_h = 150, 150
        self._max_w, self._max_h = 900, 1200
    ...
    def mouseMoveEvent(self, event):
        if self._resize_edge and self._resize_start:
            ...
            if "right" in self._resize_edge:
                w = max(self._min_w, min(self._max_w, geo.width() + delta.x()))
            if "bottom" in self._resize_edge:
                h = max(self._min_h, min(self._max_h, geo.height() + delta.y()))
