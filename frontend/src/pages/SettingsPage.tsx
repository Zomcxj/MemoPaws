...
interface AppConfig {
  ...
  show_floating_widget: boolean; shortcuts: ShortcutState; text_replacements: { abbr: string; replacement: string }[]; has_api_key?: boolean;
}
...
const DEFAULT_CONFIG: AppConfig = { theme: "dark", language: "zh", api_url: "https://open.bigmodel.cn/api/paas/v4/chat/completions", api_model: "glm-4-flash", data_dir: "", close_behavior: "tray", clipboard_max_items: 50, history_max_items: 100, show_floating_widget: true, shortcuts: DEFAULT_SHORTCUTS, text_replacements: [] };
const copy = {
...
export function SettingsPage({ theme, onThemeChange, onLanguageChange }: Props) {
  ...
  const immediate = (key: "close_behavior" | "show_floating_widget", value: CloseBehavior | boolean) => {
    update(key, value as never);
    void call(
      key === "close_behavior" ? "set_close_behavior" : "set_floating_widget_visible",
      key === "close_behavior" ? { value } : { visible: value },
    ).catch(() => {});
  };
  ...
  return <section className="settings-page">
    ...
    <Group title={t.theme}><Row><SegmentedControl className="settings-segmented" value={theme} options={[["dark", t.dark], ["light", t.light]]} ariaLabel={t.themeMode} onChange={(v) => onThemeChange(v as Theme)} /></Row></Group>
    ...
    <Group title={t.storage}><div className="settings-directory"><input aria-label={t.storage} value={dataDir} placeholder={t.storageTip} onChange={(e) => { setDataDir(e.target.value); setDirty(true); }} /><button type="button" onClick={chooseDirectory}>{t.browse}</button></div><p className="settings-hint">{t.storageTip}</p></Group>
     ...
     <Group title={t.floating}><Row><SegmentedControl value={config.show_floating_widget ? "show" : "hide"} options={[["show", t.show], ["hide", t.hide]]} onChange={(v) => immediate("show_floating_widget", v === "show")} /></Row></Group>
