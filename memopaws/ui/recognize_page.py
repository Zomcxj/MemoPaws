...
from ..ocr.ocr import OCRManager, MODE_LOCAL, MODE_CLOUD
from ..ocr.translator import SimpleTranslator
from ..canvas.canvas import CanvasWidget
from ..config.history import HistoryManager
from ..canvas.capture import ScreenCaptureOverlay
from ..core.utils import qpixmap_to_numpy, numpy_to_qpixmap, ensure_config_dir, load_svg_icon
from ..ocr.ocr_translate import OCRTranslateMixin
from ..canvas.border_glow_widget import BorderGlowWidget
...
class RecognizePage(OCRTranslateMixin, QWidget):
    ...
    def _get_pixmap_from_clipboard(self):
        ...
        if pixmap.isNull() and mime and mime.hasImage():
            image = clipboard.image()
            if not image.isNull():
                pixmap = QPixmap.fromImage(image)
        if pixmap.isNull() and mime and mime.hasUrls():
        ...
        if pixmap.isNull():
            w = self.window()
            if hasattr(w, "clipboard_page"):
                latest = w.clipboard_page.latest_image_record()
                if latest and latest.get("image_path"):
                    loaded = QPixmap(latest["image_path"])
                    if not loaded.isNull():
                        pixmap = loaded
        return pixmap

    def paste_ocr_from_clipboard(self):
    ...
    def apply_language(self, lang: str):
        ...
        self.btn_clear.setText("Clear" if lang == "en" else "清空")
        ...
        for combo in (self.translate_source_combo, self.translate_target_combo):
            ...
            # 原地更新 item 文本，避免 clear() 触发 Qt 内部字体 -1 警告
            for i in range(max(combo.count(), len(new_items))):
                if i < len(new_items):
                    if i < combo.count():
                        combo.setItemText(i, new_items[i])
                    else:
                        combo.addItem(new_items[i])
            # 移除多余 item
            while combo.count() > len(new_items):
